...
import React from 'react';
import languageData from './data'; // Make sure this contains only Hindi and English
import IconButton from '@mui/material/IconButton';
import {Fonts} from '@crema/constants/AppEnums';
import {
  useLocaleActionsContext,
  ...
  const toggleLanguage = () => {
    const newLanguage =
      locale.languageId === 'en'
        ? languageData.find((lang) => lang.languageId === 'hi')
        : languageData.find((lang) => lang.languageId === 'en');
    updateLocale(newLanguage);
  };

  const getLanguageIcon = () => {
    return locale.languageId === 'en' ? 'अ' : 'A';
  };
    ...
    >
      {!iconOnly ? (
        <IconButton
          ...
          size='large'
        >
          <span className='langIcon'>{getLanguageIcon()}</span>
          <span className='langText'>{locale.name}</span>
        </IconButton>
      ) : (
        <AppTooltip title='Language' placement={tooltipPosition}>
